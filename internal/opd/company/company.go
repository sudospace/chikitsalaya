...
const companyColumns = `id, name, address, phone, email, registration_no,
	...
	first_consultation_fee, follow_up_fee, gstin, gst_registered, fy_start_month, created_at, updated_at`
...
func scanCompany(row pgx.Row) (*Company, error) {
	...
	err := row.Scan(&c.ID, &c.Name, &c.Address, &c.Phone, &c.Email, &c.RegistrationNo,
		...
		&c.FirstConsultationFee, &c.FollowUpFee, &c.GSTIN, &c.GSTRegistered, &c.FYStartMonth, &c.CreatedAt, &c.UpdatedAt)
	if err != nil {
		return nil, err
	}
	return &c, nil
...
func (s *Store) UpdateClinic(ctx context.Context, in CompanyInput) error {
	_, err := s.Pool.Exec(ctx, `
		UPDATE clinic_settings SET
			...
			first_consultation_fee = $9, follow_up_fee = $10, gstin = $11, gst_registered = $12,
			fy_start_month = $13, updated_at = now()`,
		...
		in.GSTIN, in.GSTRegistered, in.FYStartMonth)
	return err
}
